...
def connection_db():
    ...
    print(f'read csv: {time.time()-current}')
    current = time.time()
    ...
    print(f'select plantlike: {time.time()-current}')
    current = time.time()
    ...
    print(f'string parsing: {time.time()-current}')
    current = time.time()
    ...
    # print(f'connection db finish: {time.time()-current}')
    # current = time.time()
...
def get_recommendation_top_percent(
    ...
    print(f'connection_db: {time.time()-current}')
    current = time.time()
    ...
    # for uid2, sim_score in sorted(dict(sim[uid]).items(), key=lambda x: x[1], reverse=True)[1 : top_cnt + 1]:
    ...
    print(f'score prediction: {time.time()-current}')
    current = time.time()
    # print(score_dic)
    # print(sim_dic)
    ret = []
    ...
    print(f'get reco plants: {time.time()-current}')
    current = time.time()
